...
public class ObligationValidator {
    ...
    public void validateObligationExists(String fromUser, String toUser) {
        ...
        boolean obligationExists =
            (allObligations.containsKey(fromUser) && allObligations.get(fromUser).containsKey(toUser)) ||
                (allObligations.containsKey(toUser) && allObligations.get(toUser).containsKey(fromUser));
        if (!obligationExists) {
